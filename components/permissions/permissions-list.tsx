...
import { Search, Edit, Trash2, Eye, Lock } from "lucide-react"
...
export function PermissionsList() {
  ...
  const fetchPermissions = async () => {
    ...
    try {
      ...
      const data: Permission[] = await apiClient.get(`/api/permissions?${queryParams.toString()}`)
      ...
      toast({
        title: "Lỗi",
        description: "Không thể tải danh sách quyền hạn. Vui lòng đăng nhập lại hoặc kiểm tra quyền hạn.",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!confirm(`Bạn có chắc chắn muốn xóa quyền hạn "${name}"?`)) {
      return
    }
    try {
      await apiClient.delete(`/api/permissions/${id}`)
      toast({
        title: "Đã xóa quyền hạn",
        description: `Quyền hạn "${name}" đã được xóa thành công`,
      })
      fetchPermissions()
    } catch (error: any) {
  ...
  const filteredPermissions = Array.isArray(permissions)
    ? permissions.filter((permission) => {
        ...
        if (searchTerm && !permission.displayName.toLowerCase().includes(searchTerm.toLowerCase())) return false
        return true
      })
    : []
    ...
    <div className="space-y-6 bg-gradient-to-br from-blue-50 to-white p-6 rounded-lg shadow-sm">
            ...
            <CardHeader className={`${viewMode === "list" ? "p-2" : ""}`}>
              <div className="flex items-start justify-between">
                <CardTitle className={`flex items-center ${viewMode === "list" ? "text-sm" : "text-lg"} text-blue-800`}>
                  <Lock className={`mr-2 ${viewMode === "list" ? "h-4 w-4" : "h-5 w-5"}`} />
                  {permission.displayName}
                </CardTitle>
                <Badge
                  className={`${statusColors[permission.status as keyof typeof statusColors]} ${
              ...
              {permission.description && viewMode === "grid" && <p className="text-sm text-gray-600">{permission.description}</p>}
            </CardHeader>
            <CardContent className={`${viewMode === "list" ? "p-2" : ""}`}>
              <div className={`space-y-2 ${viewMode === "list" ? "text-sm" : ""}`}>
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-gray-800">{permission.category}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className={`flex space-x-2 ${viewMode === "list" ? "justify-end" : "pt-2"}`}>
                  ...
                  <Button
                    ...
                    onClick={() => handleDelete(permission._id, permission.displayName)}
                  >
      ...
      {filteredPermissions.length === 0 && !isLoading && (
        <div className="text-center py-12">
          <p className="text-gray-600 text-lg">Không tìm thấy quyền hạn nào</p>
